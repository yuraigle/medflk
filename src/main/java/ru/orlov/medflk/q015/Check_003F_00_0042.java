...
@Component
public class Check_003F_00_0042 extends AbstractCheck {
        ...
        for (Zap zap : zlList.getZapList()) {
            if (zap.getZSl().getSankList() != null) {
                for (Sank sank : zap.getZSl().getSankList()) {
                    ...
                    boolean isMee = (sTip > 49 && sTip < 70) || sTip == 88 || (sTip > 89 && sTip < 94);
                    if (isMee && sSum.compareTo(BigDecimal.ZERO) != 0 && sIst == 1) {
                        hasSank = true;
                        break;
                    }
                }
